...
import Insurance_Circular from './Insurance_Circular';
import Insurance_News from './Insurance_News';
import Insurance_Notice from './Insurance_Notice';
import Insurance_Post from './Insurance_Post';

const Insurance = () => {
  const [post, setPost] = useState([])
    const navigate = useNavigate()
    useEffect(()=> {
        axios.get('http://localhost:3000/auth/insurance')
        .then(result => {
  ...
  return (
    <div>
      ...
      <Link to="/dashboard/insurance_post" className='btn btn-primary btn-sm' style={{ marginRight: '10px' }}>
        Post
      </Link><nbsp></nbsp><nbsp></nbsp><nbsp></nbsp>
      <Link to="/dashboard/insurance_notice" className='btn btn-primary btn-sm' style={{ marginRight: '10px' }}>
        Notice
      </Link><nbsp></nbsp><nbsp></nbsp><nbsp></nbsp>
      <Link to="/dashboard/insurance_news" className='btn btn-primary btn-sm' style={{ marginRight: '10px' }}>
        News
      </Link><nbsp></nbsp><nbsp></nbsp><nbsp></nbsp>
      <Link to="/dashboard/insurance_circular" className='btn btn-primary btn-sm' style={{ marginRight: '10px' }}>
        Circular
      </Link>
      <div className='mt-3'>
        <table style={{background:'none', color:'white'}}>
          <thead>
            <tr>
              <th style={{padding:"", border:'1px solid white'}}>post_id</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>TYPE</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>SUBTYPE</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>TITLE</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>POST</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>DESCRIPTION</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>ACTIONS</th>
            </tr>
          </thead>
          <tbody>
            {
              post.map(e => (
                <tr>
                  ...
                  <td style={{padding:"0 1rem", border:'1px solid white'}}>{e.post}</td>
                  ...
                  <td style={{padding:"1 1rem", border:'1px solid white'}}>
                  <td style={{padding:" 1rem"}}>
                    <Link to={`/dashboard/edit_employee/`+e.post_id}  className='btn btn-primary btn-sm me-2'>Edit</Link>
                  </td>
