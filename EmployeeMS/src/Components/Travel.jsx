import React, { useEffect, useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import axios from 'axios';
import Travel_Circular from './Travel_Circular';
import Travel_News from './Travel_News';
import Travel_Notice from './Travel_Notice';
import Travel_Post from './Travel_Post';


const Travel = () => {
  const [post, setPost] = useState([])
    const navigate = useNavigate()
    useEffect(()=> {
        axios.get('http://localhost:3000/auth/travel')
        .then(result => {
            if(result.data.Status){
                setPost(result.data.Result);
            } else{
                alert(result.data.Error)
            }
        }).catch(err => console.log(err))
    }, []);

    const handleDeleteConfirmation=(id)=>{
      const shouldDelete=window.confirm("Are you sure you want to delete?");
      if(shouldDelete){
        handleDelete(id);
      }
    }

    const handleDelete = (id) => {
        axios.delete('http://localhost:3000/auth/delete_post/' + id)
        .then(result => {
          if(result.data.Status){
            Window.Location.relode()
          } else{
            alert("here we got error :  "+result.data.Error)
          }
        })
      }
      
  return (
    <div>
        <div className='px-5 mt-3'>
      <div className='d-flex justify-content-center'>
        <h3>Travel</h3>
      </div>
      <Link to="/dashboard/addpost" className='btn btn-primary btn-sm' style={{ marginRight: '10px' }}>
        Add 
      </Link><nbsp></nbsp><nbsp></nbsp><nbsp></nbsp>
      <Link to="/dashboard/travel_post" className='btn btn-primary btn-sm' style={{ marginRight: '10px' }}>
        Post
      </Link><nbsp></nbsp><nbsp></nbsp><nbsp></nbsp>
      <Link to="/dashboard/travel_notice" className='btn btn-primary btn-sm' style={{ marginRight: '10px' }}>
        Notice
      </Link><nbsp></nbsp><nbsp></nbsp><nbsp></nbsp>
      <Link to="/dashboard/travel_news" className='btn btn-primary btn-sm' style={{ marginRight: '10px' }}>
        News
      </Link><nbsp></nbsp><nbsp></nbsp><nbsp></nbsp>
      <Link to="/dashboard/travel_circular" className='btn btn-primary btn-sm' style={{ marginRight: '10px' }}>
        Circular
      </Link>
      <div className='mt-3'>
        <table style={{background:'none', color:'white'}}>
          <thead>
            <tr>
              <th style={{padding:'', border:'1px solid white'}}>NO.</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>post-type</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>sub-type</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>title</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>post</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>descript</th>
              <th style={{padding:"0 1rem", border:'1px solid white'}}>ACTIONS</th>
            </tr>
          </thead>
          <tbody>
            {
              post.map(e => (
                <tr>
                  <td style={{padding:'', border:'1px solid white'}}>{e.post_id}</td>
                  <td style={{padding:"0 1rem", border:'1px solid white'}}>{e.posttype}</td>
                  <td style={{padding:"0 1rem", border:'1px solid white'}}>{e.subtype}</td>
                  <td style={{padding:"0 1rem", border:'1px solid white'}}>{e.title}</td>
                  <td style={{padding:"0 1rem", border:'1px solid white'}}>{e.post}</td>
                  <td style={{padding:"0 1rem", border:'1px solid white'}}>{e.descript}</td>
                  {/* <td>{e.category_name}</td>
                  <td>{e.location}</td>
                  <td>{(e.dob).split("T")[0]}</td>
                  <td>{e.state}</td>
                  <td>{(e.doj).split("T")[0]}</td> */}
                  {/* <td>
                    {/* <img src={`http://localhost:3000/Public/Images/`+e.image} className='employee_image' alt="img" /> */}
                    {/* <img src='D:\Employee MS\Server\Public\Images' alt='path not correct'/> */}
                    {/* <img className='employee_image' src='data:image/jpeg;base64,'>

                    </img>
                  </td>  */}
                  {/* <td>
                    <button className='btn btn-danger btn-sm me-2' onClick={()=>{

                      handleWorking(e.post_id)
                    }}>Active</button>
                  </td> */}
                  <td style={{padding:"1 1rem", border:'1px solid white'}}>
                  <td style={{padding:" 1rem"}}>
                    <Link to={`/dashboard/edit_post/`+e.post_id}  className='btn btn-primary btn-sm me-2'>Edit</Link>
                  </td>
                  <td style={{padding:"0 1rem"}}>
                    <button 
                      className='btn btn-primary btn-sm me-2' 
                      onClick={() => 
                        // console.log("delete call"+e.empId)
                      handleDelete(e.post_id)
                      }>Delete</button>
                  </td></td>
                </tr>
              ))
            }
          </tbody>
        </table>
      </div>
    </div>
    </div>
  )
}

export default Travel
